...
void testBinarySearchTree() {
    ...
    // 测试删除根节点（只有一个子节点的情况）
    bst.remove(5);
    std::cout << "Tree after removing 5 (root has a child):" << std::endl;
    bst.printTree();

    // 测试删除树中唯一的节点
    bst.remove(3);
    std::cout << "Tree after removing 3 (only child):" << std::endl;
    bst.printTree();
    ...
    // 测试异常处理
    try {
        bst.findMax();  // 测试空树时查找最大元素
    } catch (const UnderflowException &) {
        std::cout << "Caught UnderflowException when trying to find max on empty tree." << std::endl;
    }
}
